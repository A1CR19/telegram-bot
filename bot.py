    ...
    if text.startswith("🛒"):
        ...
        try:
            ...
            caption = (
                ...
                f"优惠：无\n\n"
                f"💼 收款地址(USDT-TRC20)：\n\n"
                f"THTXffejAMtqzYKW6Sxfmq8BXXz9yEHYCQ\n\n"
                f"👆 点击复制钱包, 地址尾号 EHYCQ 👆\n\n"
                f"提示：\n"
                f"- 提币后请点击（提取卡密）按钮，Bot 会发送专属提卡密令！\n"
                f"- 激活成功后，Bot 会立刻发送您的卡号卡密。"
            )
            await update.message.reply_photo(photo=CARD_IMG_ID, caption=caption, parse_mode="Markdown")
        except Exception as e:
            logging.error(f"处理商品信息失败: {e}")
            await update.message.reply_text("格式错误或数据异常，请重新点击按钮选择商品。")
